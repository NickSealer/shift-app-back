...
Los seeds crean 4 usuarios: 1 admin/supervisor y 3 users normales y 2 servicios por defecto, asociados todos entre sí. Ya se pueden usar cualquiera de ellos. Los user son para crear hojas de disponibilidad y confirmarlas, y el admin para organizar el cronograma y confirmarlo.
...
Crear el archivo `.env` y copiar las ENV vars con sus credenciales locales on [.env.example](.env.example) file
```
...
```
### Iniciando la app:
Para ingresar, los usuarios están en: [seeds.rb](db/seeds.rb) o copiarlo de aqui:
```
rails server
```
Users:
...
Estos son lo modelos implicados:
- Service
- User
- Assigment
- Availability
...
- `Service`, tiene un número total de `Availability` a través de sus usuarios registrados con el rol `user`, con es, y en base a ello se sabe cuantas hojas de disponibilidad se necesitan confirmadas para programar la hoja de turnos (`Shift`). El total de horas se calcula en base al array de horarios de sus campos como días.
